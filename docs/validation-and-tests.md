...
두 브랜치의 현재 k6 요청 구성은 동일하지 않다. `main`은 k6가 `/validate-token`을 직접 측정하는 요청을 추가로 보내지만, refactor는 보호 API만 호출. 따라서 전체 `http_req_duration`, `http_reqs`, 요청 속도를 그대로 성능 개선율로 해석하지 않음
...
`http_reqs: 1,201`은 `/api/demo/me` 1,200회와 setup 토큰 발급 1회를 합친 값. refactor에서는 매 반복마다 `/validate-token`을 호출하지 않으므로 `iterations`와 `http_reqs`가 거의 같음
...
JWKS 캐시 워밍 업 이후 auth-service가 중단되어도 보호 API가 계속 처리되는지 확인한다. 현재 스크립트는 auth-service 중단과 JWKS 캐시 워밍 시점을 자동 기록하지 않으므로, 첫 보호 요청 성공을 확인한 뒤 별도 터미널에서 auth-service를 수동으로 중단해야 함
